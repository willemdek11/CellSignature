...
remove_duplicates_cs <- function(exp,sig){
  exp <- pp_exp(exp, sig)
  sig <- pp_sig(exp, sig)

  fg_duplicates <- sig[duplicated(sig$gene) | duplicated(sig$gene, fromLast=TRUE),] %>% droplevels()
  fg_all <- anti_join(sig, fg_duplicates, by = c("gene" = "gene"))

  cor_data <- fastCor_cs(t(exp))

  for (i in levels(fg_duplicates$gene)){
    corr_max <- 0
    for (j in fg_duplicates[fg_duplicates$gene == i, "cell"]){
      corr_subb <- pp_cor(cor_data, sig, j)
      corr_temp <- (sum(corr_subb[,i]))/length(corr_subb[,i])
      if (corr_temp > corr_max){
        corr_max <- corr_temp
        type_max <- j
      }
    }
    fg_subb <- data.frame(i, type_max)
